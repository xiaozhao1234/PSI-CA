...
namespace PSI {
	...
	void PsiReceiver::run(int m, PRNG& prng, Channel& ch, block commonSeed, const u64& senderSize, const u64& receiverSize, const u64& height, const u64& logHeight, const u64& width, std::vector<block>& receiverSet, const u64& hashLengthInBytes, const u64& h1LengthInBytes, const u64& bucket1, const u64& bucket2) {
		...
		//
		auto cloudNumber = 8;
        std::cout << "receiver cloudNumber="<<cloudNumber << std::endl;
		...
		// std::cout << "User set transformed\n";
		// finish = clock();
		// double duration;
		// duration = (double)(finish - start1) / CLOCKS_PER_SEC;
		// std::cout << "User time:"<< duration <<"\n";
		timer.setTimePoint("User set transformed");
///////////////////////////////////////////////////////
		...
		timer.setTimePoint("Cloud set transformed");
		...
		for (auto low = 0; low < receiverSize; low += bucket2) {
			...
			for (auto j = low; j < up; ++j) {
				...
				aesInput[j] = *(block*)hashOutput;
				recvSet[j] = *(block*)(hashOutput + sizeof(block));

				// allHashes[*(u64*)hashOutput].push_back(std::make_pair(*(block*)hashOutput, j));
			}

			commonAes.ecbEncBlocks(aesInput, up-low, aesOutput);
			for (auto i = low; i < up; ++i) {
				recvSet[i] ^= aesOutput[i];
				allHashes[*(u64*)(&recvSet[i])].push_back(std::make_pair(recvSet[i], i));
		...
		block* recvBuff = new block[bucket2 * sizeof(block)];
		...
		for (auto low = 0; low < senderSize; low += bucket2) {
			...
			ch.recv(recvBuff, (up - low) * sizeof(block));

			for (auto idx = 0; idx < up - low; ++idx) {
				u64 mapIdx = *(u64*)(recvBuff + idx * sizeof(block));

				auto found = allHashes.find(mapIdx);
				if (found == allHashes.end()) continue;

				for (auto i = 0; i < found->second.size(); ++i) {
					if (memcmp(&(found->second[i].first), recvBuff + idx * sizeof(block), sizeof(block)) == 0) {
						++psi;
						break;
					}
				}
